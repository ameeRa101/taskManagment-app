...
export const AddNewTask = ({
  ...
  async function UpdateTask() {
    setIsSubmitting(true);
    try {
      const response = await axios.put(`https://dummyjson.com/todos/${editingTask.id}`, {
        todo: title,
        description,
        dueDate,
        priority,
      ...
      <form onSubmit={submitHandler}>
        ...
        <div className="form-buttons">
          <button disabled={isSubmitting} type="submit">
            {isSubmitting ? <i className="fa-solid fa-circle-notch" /> : editingTask ? "Update Task" : "Add Task"}
          </button>
          <button disabled={isSubmitting} type="button" onClick={() => setShow(false)}>
            Cancel
          </button>
        </div>
      </form>
